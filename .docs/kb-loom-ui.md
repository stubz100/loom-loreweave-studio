...
| **2** | **Project dialogs** ✅ **DONE 2026-09-20** (journal "🗂 M2.14 step 2"): `tauri-plugin-dialog` folder picker for Open/New (Browse… in the shell); the New-project modal with the footprint estimate beside the size cap; the **Start screen** in the canvas with recent projects as cards; every native call has a typed-path fallback so the browser dev build keeps working; the last `window.prompt` is gone. Thumbnails on the cards wait for a `/projects` cover field (backend, later) | "painful project open" fixed | dev-build fallback to the typed path; a recents screen with thumbnails |
| **3** | **Composer panel** — **3a ✅ DONE 2026-09-20** (journal "✦ M2.14 step 3a"): the Cast / Sandbox composer as a column — pipeline + variant + sampling preset, the prompt with the flux.2-dev JSON tree as a form, count or candidates, style chip + apply, catalog-driven Advanced, **Preview** (dry-run modal) and **Generate/Cast pinned at the foot**; compose state persisted so authored text survives a reload; the request is built exactly as v1 built it (pinned by a test + a live dry-run). **3b ✅ DONE 2026-09-20** (journal "✦ M2.14 step 3b"): Expand mode from stage B on — hero + anchor head, recipe preset + the cell picker with pose icons (subset → `cells`), pipeline / variant / flux2 sampling + directive prompting, strength + realize with Matte hero (mixed disabled without the mask), identity with its four rules as text, the character clause **per asset**, style (default off), the video sketch folded, Advanced (img2img mode), **Preview** (Stage-B dry-run modal) + **Generate dataset** pinned; the body is built as v1's `buildStageBBody` built it and its keys are pinned against the server's `extra=forbid` model. Train mode on the Panel's Train tab: base / init / trigger / steps + advanced, **Stage the run** pinned, staged runs with Add to queue and an inline two-click remove. The selected character's detail now lives in the app store (canvas, header, composer, inspector share it; the poller keeps it current) and the Info tab gained **Star as hero** until step 5's canvas actions | the grid becomes the canvas | every control reachable in its new home; Stage-B recipe fires the same request (the pre-flight modal proves it byte-for-byte) |
| **4** | **Inspector tabs** — **✅ DONE 2026-09-20** (journal "🔎 M2.14 step 4"): `src/inspect/{InfoTab,PostTab,VersionTab}.tsx`. **Info** = the tile's facts, pose, identity, prompt/params/log folded, plus its actions (Star as hero · Set as face anchor · Face portrait · Re-run with seed/steps/guidance). **Post** = the selected image's stack as a **tree** (a step hangs under the image it reads, branches indented, tombstones struck through, live status from the queue), View / Queue / Retry / two-click Remove on any leaf, and the add form laid out as a column (preset · backend · Reads = branch point · model · strength with the effective-steps readout · prompt or the flux.2-dev JSON tree · negative · restyle + style · size row); the body keys are pinned against the server's `extra=forbid` models. **Version** = the active version's facts, switch / New / Finalize-Unlock, the prompt template with Save, hero + anchor with Clear; it is also what Info shows when nothing is selected. Stacks live in the app store and refresh on the poller's tick. **Readiness** stays a placeholder until step 6 | the stack gets room | the JSON tree renders as a form, not a stack of stacked inputs |
| **5** | **Canvas** — **✅ DONE 2026-09-20** (journal "🖼 M2.14 step 5"): `src/canvas/{tiles,actions,registry,Tile,Grid,Grouped,Loupe}`. The tile model mirrors v1's flattening (multi pool → per-output, a running cast's interim candidates + placeholder, tombstones draw nothing, Curate adds the durable refs and the shot/angle/expression/rejected filters); the Sandbox scope is fixed (requester = project id). Uniform tiles with **Fit / Fill**, affordances on hover **and** on the selected tile (star · keep/remove · reject/un-reject · mark · loupe · cancel · delete), every destructive action a second click (tile, selection bar, group). **Keyboard by visual row** (columns measured from the grid), k / x / space / Del×2 / Enter / Home / End, `?` lists them. **Grouped** view ported (operation groups, chains left to right, folded cards). **Loupe** replaces the canvas: prev / next, 1:1, facts strip, **Compare** pins one image beside the current (c). The Strip carries the Curate filters + counts and the selection bar (n marked · Keep · Reject · Delete · Clear) | the review loop | keyboard stride correct on any width; compare two tiles |
| **6** | **Train** — **✅ DONE 2026-09-20** (journal "⚙ M2.14 step 6"): the staging form stays in the composer's Train tab (step 3b) and now reports "n staged, n active, n finished" with **Show in the dock**; **staged runs + trainer jobs live in the Dock's Training pane** (Active · Training · Recent filters; rows carry step / loss / ETA from `job.note`; Add to queue · Cancel · **Preview** · **Promote** · Cleanup · Remove, every destructive one a second click); the **Captions view** on the canvas (thumbnail beside the caption, origin badge, no-trigger warning, Save / Discard / Reset per row, Reset all on a second click, the coverage filters apply); the **Readiness tab** with the four tiers' details inline (missing cells as chips, duplicate groups and outliers as thumbnails, the on-model scan closing through the one poller); the **LoRA preview form** in the composer (pose icons, prompt override, seed, size = trained by default, weight, Render / A/B vs base) | Stage D stops being a card in a column | the same endpoints, new homes; dock shows step/loss/ETA from `job.note` |
| **7** | **Edit mode** — **✅ DONE 2026-09-21** (journal "🎨 M2.14 step 7"): `canvas/Edit.tsx` = the selected image fills the canvas with a tool rail (Brush · Eraser · Lasso · size · feather · Invert · Clear · From matte) and the mask as an amber overlay; **Save mask** exports white-on-black (feathered) and `POST /outputs/masks` stores it under `out/masks/` atomically (PNG-checked, size must match the image); the **Inpaint** postproc preset (`_PP_PRESETS["inpaint"]`: sd35 default, zimage allowed, strength 0.95, mask required and checked, source dims kept) queues a batch inpaint job with `init_image` + `mask_image` on the item; the Post tab offers Inpaint with the saved masks and a *Paint a mask* button; `e` opens Edit, Escape closes it (a lasso eats the first Escape), strokes survive leaving the view. Dry-run verified by test (the planned job carries the mask). flux2 masked-denoise stays rig-owed | the tool the author asked for first | mask round-trips to disk and into a step's `mask`; queue is dry-run-verified |
| **8** | **World + placeholders** — **✅ DONE 2026-09-21** (journal "🌍 M2.14 step 8"): the **World workspace** per §3.8 — the Panel lists (Styles with thumbnails and + Style · the prose Outline · the Spine characters with their profile link · the Pose sets) and the **canvas hosts the editor** (`src/world/`: the style editor with Save / Set as default / Delete on a second click / the apply-by-default gate / the sample with Generate · Cancel · Delete, the world prose, the spine with premise + character rows + Make profile / Re-sync / Open profile, the pose icons with Generate missing · Redo all · per-icon Redo / Delete); the style sample and the icon jobs close through the one poller; L1 shares the toasts. The **later workspaces** (Shots · Flow · Episode) are tabs badged with their phase that open on a **card** naming what their panel, canvas, inspector and dock will hold (§3.10); Muse stays a disabled tab. **The migration table is complete.** |
...
**Close-out (2026-09-21).** Rows 0–8 are built in `frontends/v2` (M2.14, journal entries "🧱 step 1" through "🌍 step 8"), every step pinned by `orchestrator/tests/test_v2_frame.py` (52 source contracts) plus the TestClient suites the steps touched. What the plan promised and v2 now has: the triad frame with layout memory; toasts + banners everywhere (the L1 blind spot closed); the Composer column (Cast · Expand · Train) with Preview; the Inspector tabs (Info · Post as a tree · Readiness · Version); the responsive canvas with keyboard by visual row, Grouped, Loupe + Compare, Captions, **Edit mode with a real mask painter** and the Inpaint pass it needed on the backend; the dock's Jobs pane with training rows; the World editors; no native dialogs. **Not yet: the author's click-through** — nothing in v2 has been driven by a person; the first launch (`loom-dev`, v2 + Tauri by default) is the acceptance. **v1 retirement criteria:** (1) the author has run the daily loop in v2 (Sandbox flux.2-dev JSON cast → refine → mask + Inpaint) and the bootstrap loop (Cast → star → Expand → Curate → Train → preview) on the rig once the GPU is back; (2) the three owed step-5 items (virtualisation past ~300 tiles · Ctrl+P go-to-asset · compare two marked tiles) are built or waived; (3) `frontends/v1` is deleted with its contract tests re-pointed to v2 in one commit, and the shell's `dev` script points at v2. Until then v1 stays frozen (`npm run dev` in the shell) as the fallback.
