...
│   └── graph_index/             # GraphRAG/vector index (R170 deferral lifted 2026-07-05; P6 backstop)
├── jobs/
...
| **P6 Polish — post-v1, NO essential deliverables (R165)** | `kb-loom-p6.md` | **Not required for alpha/v1 — P5 is the v1 gate (R165).** Deferred polish: **all 3D (`trellis2` props/scenery GLBs + proxy-miniature depth-ControlNet route, R128)**; **effect plugin API (Python/JS/TS)** (§6.7); **engine/DCC export — assets *and* graph to Unreal 5.7+/Blender** (§6.7); **GraphRAG retrieval index** (R138/R170 — deferral lifted 2026-07-05; P6 = backstop) / video LoRAs; **+ any unforeseen overflow from P0–P5** (flexible scope). *(Flux2-deep / LTXV-extend-harden / multi-LoRA+style / postproc-expansion MOVED → P5, R147.)* | post-v1; **nothing here gates v1** |
...
| **R138** | **Project context text-only v1** | Project-wide context = **text-structured** for v1 (world/style/cast/rules/synopsis). The embedding **model ships in P4 for scoring** (R137); only the **persistent GraphRAG/retrieval index** comes later (hard deferral lifted 2026-07-05, R170) — it **reuses the same model** (no new model later, just the graph/vector index build) (`kb-loom-p4.md` §13; refined by R170). |
...
| **R170** | **GraphRAG posture** *(amended 2026-07-05: deferral LIFTED)* | Long-term retrieval stays **GraphRAG-style**: typed project facts + stable IDs + graph/vector retrieval, not plain vector-only RAG. **P2/P4 write graph-ready artifacts now** (`training_context.json`, `caption_policy.json`, `project_facts.jsonl`). **Amended by the author 2026-07-05:** the original post-v1/P6 deferral of the **persistent index build/query** is **removed** — the index may be built as soon as it is useful; **P6 remains the backstop home** if it has not landed earlier. It must still never block LoRA training or alpha. |
