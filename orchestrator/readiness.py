...
ONMODEL_OUTLIER_FLOOR = 0.25      # a ref scoring below max(floor, mean−0.15) is an outlier
...
def _dupes_tier(refs_dir: Path, refs: list[dict]) -> dict:
    ...
    for ref in refs:
        ...
        if h is None:
            ...
            hashes[ref["id"]] = h
    # greedy clustering — ref sets are ~30 items, pairwise is fine
    ids = list(hashes)
...
def harvest_score_job(vdir: Path, version: dict, job: dict) -> dict:
    ...
    known = {r["id"] for r in version.get("ref_set") or []}
    rows = []
    for it in data.get("items") or []:
        meta = it.get("meta") or {}
        rid = meta.get("ref_id")
        if it.get("status") == "ok" and rid in known:
            rows.append({
                "ref_id": rid,
    ...
    outliers = []
    if mean is not None:
        floor = max(ONMODEL_OUTLIER_FLOOR, mean - 0.15)
        outliers = [r["ref_id"] for r in scored if r[metric] < floor]
    if not faces:
