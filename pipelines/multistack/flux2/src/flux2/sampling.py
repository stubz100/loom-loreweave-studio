...
def denoise(
    ...
    for t_curr, t_prev in tqdm(steps, desc="Denoising", unit="step"):
...
def denoise_cached(
    ...
    steps = list(enumerate(zip(timesteps[:-1], timesteps[1:])))
    for step_idx, (t_curr, t_prev) in tqdm(steps, desc="Denoising (cached)", unit="step"):
        t_vec = torch.full((img.shape[0],), t_curr, dtype=img.dtype, device=img.device)
...
def denoise_cfg(
    ...
    steps = list(zip(timesteps[:-1], timesteps[1:]))
    for t_curr, t_prev in tqdm(steps, desc="Denoising (CFG)", unit="step"):
        t_vec = torch.full((img.shape[0],), t_curr, dtype=img.dtype, device=img.device)
