...
# Windows ROCm's single-GPU torch build exposes a partial torch.distributed module.
# Accelerate calls is_initialized() during cleanup even though no process group exists.
if getattr(torch, "distributed", None) is not None and not hasattr(torch.distributed, "is_initialized"):
